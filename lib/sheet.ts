...
export const SHEET_ID = "1Ust7i7HhsPrJdQxJ3v-GvhulQjfC_eCJxXCHuFxDvbc";
 ...
 */
export const SHEET_NAMES = ["Análisis", "Analisis", "análisis", "analisis"];

export const csvUrl = (hoja: string) =>
  `https://docs.google.com/spreadsheets/d/${SHEET_ID}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(hoja)}`;
...
/** Tope de equipos: la competencia son diez, con margen por si se suma alguno. */
 ...
 */
export async function equiposDelSheet(): Promise<string[]> {
  for (const hoja of SHEET_NAMES) {
    const corte = new AbortController();
    const reloj = setTimeout(() => corte.abort(), 8000);
    try {
      const res = await fetch(`${csvUrl(hoja)}&cachebust=${Date.now()}`, {
        cache: "no-store",
        signal: corte.signal,
      });
      if (!res.ok) continue;
